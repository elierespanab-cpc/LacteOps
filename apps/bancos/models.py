...
class CuentaBancaria(AuditableModel):
    ...
    numero_cuenta = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Número de Cuenta"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(saldo_actual__gte=0), name="chk_saldo_positivo_cuenta"
            )
        ]
...
class MovimientoCaja(models.Model):
    ...
    cuenta = models.ForeignKey(
        CuentaBancaria, on_delete=models.PROTECT, verbose_name="Cuenta"
    )
    tipo = models.CharField(
        max_length=25, choices=TIPO_CHOICES, verbose_name="Tipo de Movimiento"
    )
    ...
    monto_usd = models.DecimalField(
        max_digits=18, decimal_places=2, verbose_name="Monto en USD"
    )
...
class TransferenciaCuentas(AuditableModel):
    ...
    monto_origen = models.DecimalField(
        max_digits=18, decimal_places=2, verbose_name="Monto Origen"
    )
    monto_destino = models.DecimalField(
        max_digits=18, decimal_places=2, verbose_name="Monto Destino"
    )
    tasa_cambio = models.DecimalField(
        max_digits=18,
        decimal_places=6,
    ...
    def ejecutar(self):
        ...
        if self.estado != "PENDIENTE":
            raise EstadoInvalidoError(
                "Transferencia de Cuentas", self.estado, "ejecutar"
            )
        ...
        with transaction.atomic():
            ...
            registrar_movimiento_caja(
                ...
                if self.cuenta_origen.moneda == "VES"
            ...
            registrar_movimiento_caja(
                ...
                tasa_cambio=tasa_cambio
                if self.cuenta_destino.moneda == "VES"
                else Decimal("1.000000"),
                referencia=referencia,
        ...
        logger.info(
            ...
            self.numero,
            self.cuenta_origen.nombre,
            monto_origen,
            self.cuenta_origen.moneda,
            self.cuenta_destino.nombre,
            monto_destino,
            self.cuenta_destino.moneda,
        )
    ...
    def anular(self):
        ...
        logger.info(
            "Transferencia %s ANULADA. Movimientos inversos generados.", self.numero
        )
...
class PeriodoReexpresado(AuditableModel):
    ...
    tasa_cierre = models.DecimalField(
        max_digits=18, decimal_places=6, verbose_name="Tasa de Cierre (VES/USD)"
    )
    ejecutado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, verbose_name="Ejecutado por"
    )
...
class MovimientoTesoreria(models.Model):
    ...
    monto_usd = models.DecimalField(
        max_digits=18, decimal_places=2, editable=False, verbose_name="Monto en USD"
    )
    categoria = models.ForeignKey(
        "core.CategoriaGasto", on_delete=models.PROTECT, verbose_name="Categoría"
    )
